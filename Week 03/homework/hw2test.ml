...
type english_toy_nonterminals =
  | TP | CP | DP | VP | V | Noun | D

let english_toy_grammar =
  (TP,
   function
     | TP ->
        [[N DP; N VP];
...
(* simplest test *)
let test0 =
  ((parse_prefix english_toy_grammar accept_all ["cats"]) = None)

(* simple test *)
let test1 =
  ((parse_prefix english_toy_grammar accept_all ["cats";"drink";])
   = Some ([(TP, [N Noun; N V]);(Noun, [T"cats"]);(V, [T"drink"])],[]))

(* testing full sentence derivation *)    
let test2 =
  ((parse_prefix english_toy_grammar accept_all ["the";"cat";"drinks";"the";"pepsi";"."])
   = Some ([(TP, [N DP; N VP]);(DP, [N D; N Noun]);(D, [T "the"]);
          (Noun, [T "cat"]);(VP, [N V; N DP]);(V, [T "drinks"]);
          (DP, [N D; N Noun]);(D, [T "the"]);(Noun, [T "pepsi"])],["."]))

(* testing empty string values*)    
let test3 =
  ((parse_prefix english_toy_grammar accept_all ["the";"cat";"drinks";"";"pepsi";"."])
   = Some ([(TP, [N DP; N VP]);(DP, [N D; N Noun]);(D, [T "the"]);
            (Noun, [T "cat"]);(VP, [N V; N DP]);(V, [T "drinks"]);
            (DP, [N D; N Noun]);(D, [T ""]);(Noun, [T "pepsi"])],["."]))

(* testing word ambiguity *)
let test4 =
  ((parse_prefix english_toy_grammar accept_all ["the";"drink";"drinks";"";"pepsi";"."])
   = Some ([(TP, [N DP; N VP]);(DP, [N D; N Noun]);(D, [T "the"]);
            (Noun, [T "drink"]);(VP, [N V; N DP]);(V, [T "drinks"]);
            (DP, [N D; N Noun]);(D, [T ""]);(Noun, [T "pepsi"])],["."]))
